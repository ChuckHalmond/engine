...
class QuadGeometry extends GeometryBase {
	...
	toBuilder(): GeometryBuilder {
		...
		/*new class extends GeometryBuilder {
		...
		for (let wi = 0; wi < widthSegment; wi++) {
			...
			builder.addVertex([vertexPosition[0] + segmentVertexWidth, vertexPosition[1], vertexPosition[2]]);
			uvs.push([uvPosition[0] + segmentUvWidth, uvPosition[1]]);
			for (let hi = 0; hi < heightSegment; hi++) {
				const verticesCount = vertices.length;
				builder.addVertex([vertexPosition[0] + segmentVertexWidth, vertexPosition[1] + segmentVertexHeight, vertexPosition[2]]);
